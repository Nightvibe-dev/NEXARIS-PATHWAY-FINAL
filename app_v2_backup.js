...
function cargarCorreo() {
    ...
    const perfil = JSON.parse(localStorage.getItem("perfilNexaris"));
    ...
    const nombreSidebar = document.getElementById("nombreSidebar");
    ...
    if (email) {
        email.textContent = perfil.correo;
    }

    if (usuarioTop) {
        usuarioTop.textContent = "👤 " + perfil.nombre;
    }

    if (nombreSidebar) {
        nombreSidebar.textContent = perfil.nombre;
    }
